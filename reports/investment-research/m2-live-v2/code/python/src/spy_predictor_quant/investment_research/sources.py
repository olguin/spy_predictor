...
def adapter_kind(source: dict) -> str:
    if source.get("adapter", "document") != "document":
...
def normalized_data(raw: bytes, source: dict, symbol: str, submissions: dict | None = None) -> dict | None:
    kind = adapter_kind(source)
    if kind == "alpaca_daily":
        from spy_predictor_quant.meta_analysis import daily_metrics
        payload = json.loads(raw)
        if payload.get("symbol") != symbol:
            raise ValueError("Market bar symbol mismatch")
        data = daily_metrics(payload, {"id": source["source_id"], "kind": kind,
            "retrieved_at": source["captured_at"], "data_end": source.get("data_end") or source["captured_at"],
            "adjustment": source["share_basis"], "feed": source["feed"]}, datetime.now(timezone.utc))
        data.update(symbol=symbol, return_basis="price_return", interpretation="Descriptive completed sessions, not a prediction or event causality")
        return data
    if kind == "etf_holdings":
        from spy_predictor_quant.meta_evidence import parse_holdings_csv
        return parse_holdings_csv(raw, symbol, datetime.now(timezone.utc))
    if kind == "etf_profile":
        from spy_predictor_quant.meta_evidence import parse_etf_profile
        data = parse_etf_profile(json.loads(raw), symbol, datetime.now(timezone.utc))
        data["metric_as_of"] = json.loads(raw).get("metric_as_of", {})
        return data
    if kind == "fred_csv":
        from spy_predictor_quant.meta_analysis import parse_fred
        rows = [(d, v) for d, v in parse_fred(raw, source['series_id']) if d <= datetime.now(timezone.utc).date()]
        if not rows:
            raise ValueError("Empty macro series")
        return {"series_id": source['series_id'], "units": source['units'],
                "observations": [{"date": d.isoformat(), "value": v} for d, v in rows[-12:]],
                "qualification": "Current-vintage observations; release surprise and historical availability are not established"}
    if kind == "sec_submissions":
        payload = json.loads(raw)
...
def evidence_number(evidence: dict, pointer: str):
    ...
    if pointer == '/result_decimal' and evidence.get('kind') == 'calculation':
        from decimal import Decimal
        if not Decimal(value).is_finite():
            raise ValueError('Nonfinite calculation reference')
        return value
