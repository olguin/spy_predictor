...
def validate_mandate(mandate):
    ...
    if mandate.get('publication_policy'):
        required = set(mandate['publication_policy']['refresh_source_ids'])
        if not required <= sources.keys():
            raise ValueError('Unknown publication refresh source')
        if not any(sources[k]['kind'] in {'policy', 'macro', 'company'} for k in required):
            raise ValueError('Publication refresh requires event source coverage')
        event_scope = {symbol for k in required if sources[k]['kind'] in {'policy', 'macro', 'company'}
                       for symbol in (sources[k]['symbols'] or symbols)}
        if not set(symbols) <= event_scope:
            raise ValueError('Every instrument requires declared event refresh coverage')
        prices = {symbol for k in required if sources[k]['adapter'] == 'alpaca_daily' for symbol in sources[k]['symbols']}
        paired_symbols = set(symbols) | {i[k] for i in mandate['watchlist'] for k in ('benchmark', 'sector_benchmark')}
        if not paired_symbols <= prices:
            raise ValueError('Publication refresh requires prices for every watchlist instrument')
...
def finish(controller, state):
    ...
    for row in final['role_coverage']:
        if row['status'] == 'omitted':
            controller.store.event(state, 'ROLE_OMITTED', role=row['role'], reason=row['reason'])
    ...
    for row in final['instruments']:
        ...
        from .rendering import condition_line
        lines += ['', 'Review conditions:'] + [condition_line(controller.store, c) for c in row['review_conditions']]
        lines += ['', 'Claims:']
