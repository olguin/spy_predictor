...
def source_identity() -> dict:
    ...
    paths += list((ROOT / 'python/src/spy_predictor_quant/investment_research/web').glob('*'))
    import hashlib
...
class Controller:
    ...
    def create(self, mandate: dict) -> dict:
        ...
        with self.store.lock():
            ...
            state['telemetry_version'] = 'investment-research-telemetry-v1'
            if mandate.get('publication_policy'):
                count = len(mandate['publication_policy']['refresh_source_ids'])
                state['publication_reservations'] = {'source_requests': count, 'tool_calls': count}
            state['run_deadline_at'] = (datetime.fromisoformat(state['started_at']) +
                timedelta(seconds=mandate['budgets']['wall_seconds'])).isoformat()
            if is_m2(mandate):
                state['runtime_policy_version'] = 'investment-research-m3-v1'
            ...
            self.store.event(state, "BUDGET_RESERVED", reservations=state['reservations'],
                             ...
                             publication=state.get('publication_reservations', {}))
            return state
    ...
    def add_task(self, state, role, stage, question, *, round_number=0, parent=None, question_id=None):
        ...
        state['events'].append({'at': utc_now(), 'kind': 'TASK_CREATED',
            'task_id': task['task_id'], 'parent_task_id': parent,
            'model_turn_limit': task['model_turn_limit'],
            'worker_timeout_seconds': state['mandate']['runtime']['timeout_seconds']})
        return task
    ...
    def run(self) -> dict:
        with self.store.lock():
            ...
            if state["status"] in {"DRAFT", "INCOMPLETE", "INTERRUPTED", "FAILED", "PUBLISHED"}:
                return state
            if any(a["status"] in {"STARTED", "RECEIVED", "FAILED"} for a in state["attempts"]):
                last_observed = state.get('updated_at')
                for attempt in state["attempts"]:
                    if attempt["status"] in {"STARTED", "RECEIVED", "FAILED"}:
                        attempt["status"] = "INTERRUPTED"
                        attempt.setdefault('completed_at', last_observed)
                        attempt['terminal_time_uncertain'] = True
                state["status"] = "INTERRUPTED"
                state['completed_at'] = last_observed
                state['terminal_time_uncertain'] = True
                state['interruption_detected_at'] = utc_now()
                state["usage_unknown"] = True
                self.store.event(state, "INTERRUPTED_ATTEMPT_NOT_RETRIED")
                for task in state["tasks"]:
                    if task["status"] in {"PENDING", "RUNNING"}:
                        task["status"] = "INTERRUPTED"
                        task['completed_at'] = last_observed
                        task['terminal_time_uncertain'] = True
            ...
            try:
                ...
                state["status"] = "INTERRUPTED" if isinstance(error, (KeyboardInterrupt, SystemExit)) else "INCOMPLETE"
    ...
    def execute_task(self, state, task):
        ...
        while task["status"] == "RUNNING":
            ...
            if is_m2(state['mandate']):
                from .evidence_transport import prepare
            ...
            timeout = min(runtime['timeout_seconds'], remaining_seconds(state))
            attempt['worker_deadline_at'] = (datetime.fromisoformat(attempt['started_at']) + timedelta(seconds=timeout)).isoformat()
            attempt['manifest_id'] = task['manifest_id']
            state["attempts"].append(attempt)
            self.store.event(state, 'ATTEMPT_STARTED', attempt_id=attempt['attempt_id'], task_id=task['task_id'], worker_deadline_at=attempt['worker_deadline_at'])
            try:
                response = self.worker(request, runtime, timeout)
                attempt["response_id"] = self.store.put("responses", response)
                attempt['worker_completed_at'] = utc_now()
                ...
                attempt['completed_at'] = utc_now()
                attempt['failure_type'] = type(error).__name__
                self.store.event(state, 'ATTEMPT_FAILED', attempt_id=attempt['attempt_id'], failure_type=type(error).__name__)
                raise
