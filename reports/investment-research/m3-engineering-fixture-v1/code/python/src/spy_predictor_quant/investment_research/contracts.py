...
def is_m2(mandate: dict) -> bool:
    return mandate.get("schema_version") in {"investment-research-mandate-v3", "investment-research-mandate-v4"}


def validate(name: str, value: dict, *, m2: bool = False) -> None:
    definition = schema(name)
    if name in {"mandate", "product"} and value.get("schema_version") in {f"investment-research-{name}-v2", f"investment-research-{name}-v3", f"investment-research-{name}-v4"}:
