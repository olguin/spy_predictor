...
def finish(controller, state):
    ...
    lines += ['', '## Validated numerical evidence', '', 'Values below are rendered from stored evidence, not regenerated by an agent.', '']
    import json
    for identity in state['evidence_ids']:
        e = controller.store.get('evidence', identity)
        if e.get('data') is not None or e['kind'] == 'calculation':
            data = e.get('data') if e.get('data') is not None else e
            lines += [f'### [{e.get("source_id", identity[:12])}](evidence/{identity}.json)', '', '```json', json.dumps(data, indent=2), '```', '']
