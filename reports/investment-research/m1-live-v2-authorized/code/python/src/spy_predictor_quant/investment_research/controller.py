...
from .contracts import ROOT, ROLES, TOOLS, PROMPT_VERSION, WORKER_PROTOCOL, validate, validate_findings
...
def source_identity() -> dict:
    ...
    paths += list((ROOT / f"prompts/investment-research/{PROMPT_VERSION}").glob("*"))
...
class Controller:
    ...
    def create(self, mandate: dict) -> dict:
        ...
        if len(mandate["watchlist"]) != 1 or mandate["watchlist"][0]["kind"] != "company":
        ...
        with self.store.lock():
            ...
            state = {"schema_version": "investment-research-run-v1", "run_id": str(uuid.uuid4()),
                ...
                "runtime_policy_version": "investment-research-m1-v2", "followup_round": 0,
            ...
            self.add_task(state, "director", "planning", "Define the decision. Assign neutral, material research questions to Company and Macro using ask_specialist, then submit an initial research plan.", round_number=-1)
    ...
    def add_task(self, state, role, stage, question, *, round_number=0, parent=None, question_id=None):
        task = {"task_id": f"task-{len(state['tasks']) + 1}", "role": role, "stage": stage,
                ...
                "prompt_version": f"investment-research/{PROMPT_VERSION}",
            ...
            try:
                while True:
                    ...
                    if state["phase"] == "research":
                        ...
                        if not {"company", "macro"} <= completed_roles:
        ...
        while task["status"] == "RUNNING":
            ...
            request = {"schema_version": WORKER_PROTOCOL, "task_id": task["task_id"],
                ...
                "context": {"mandate": {k: state["mandate"][k] for k in ("watchlist", "objective", "horizon_sessions", "mode")},
                    "task": {k: task[k] for k in ("task_id", "role", "stage", "question")},
                    "manifest_id": task["manifest_id"],
                    "evidence": {i: self.compact_evidence(self.store.get("evidence", i)) for i in task["visible_evidence_ids"]},
                ...
                "tool_schemas": {k: v for k, v in json.loads((ROOT / f"prompts/investment-research/{PROMPT_VERSION}/tools.json").read_text()).items()
                                 if k in task["permitted_tools"]}}
    ...
    def action(self, state, task, action):
        ...
        if name == "submit_findings":
            ...
            validate_findings(args, set(task["visible_evidence_ids"]), claims, {q["question_id"] for q in state["questions"]})
    ...
    def finish(self, state):
        ...
        product = {"schema_version": "investment-research-product-v1", "run_id": state["run_id"],
