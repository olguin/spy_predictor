...
def inventory(mandate: dict) -> dict:
    ...
    return {"schema_version": "investment-research-capabilities-v1",
            ...
            "credentials": {name: {"environment_present": all(bool(os.environ.get(k)) for k in keys),
                                   ...
                                   "research_broker_adapter": ("SEC_CURRENT_DOCUMENTS_AND_FACTS" if name == "sec" else
                                       {"alpaca": "REGISTERED_DAILY_BARS_AND_CAPTURE_IMPORT", "fred": "REGISTERED_PUBLIC_CSV"}.get(name, "NOT_IMPLEMENTED")
                                       if mandate.get("schema_version") == "investment-research-mandate-v3" else "NOT_IMPLEMENTED")} for name, keys in groups.items()},
