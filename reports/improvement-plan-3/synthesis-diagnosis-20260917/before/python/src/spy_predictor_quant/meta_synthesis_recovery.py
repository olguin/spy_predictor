...
def prepare(source: Path, output: Path, model: str | None = None) -> Path:
    ...
    run = create_immutable_run_directory(output, "synthesis-recovery")
    ...
    core = {"source_run": str(source.resolve()), "source_report_sha256": file_sha256(report_path),
        ...
        "runner": runner, "timeout_seconds": 900, "attempts": 1,
...
def main():
    ...
    if args.command == "prepare":
        ...
        path = prepare(args.source, args.output, args.model)
    else:
        if not args.run:
