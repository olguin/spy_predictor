...
def diagnostic_request(original: dict, symbol: str | None = None) -> dict:
    ...
    symbol = symbol or sorted(numerical["symbols"])[0]
    if symbol not in numerical["symbols"]:
        raise ValueError("Diagnostic symbol must exist in the frozen parent")
...
def prepare(source: Path, output: Path, model: str | None = None, *, diagnostic: bool = False,
            concise: bool = False, symbol: str | None = None) -> Path:
    ...
    if symbol is not None and not diagnostic:
        raise ValueError("Symbol selection is only permitted for a small diagnostic")
    request = diagnostic_request(original, symbol) if diagnostic else compact_request(original)
    if concise:
        if diagnostic:
            raise ValueError("Concise mode is a full-input synthesis variant")
        request["instructions"] += (
            "\nKeep this response concise: use at most one claim per symbol and at most 160 characters "
            "per prose field. Do not repeat the packet, specialist claims, or numerical tables in prose. "
            "All five assessments, all three horizons per assessment, and all 15 golden decision rows "
            "are still required. Copy exact frozen values and actions; do not round probabilities. "
            "Use only critic-accepted support. Explicit UNKNOWN/INSUFFICIENT_EVIDENCE is appropriate "
            "where the accepted evidence does not support a view.")
        request["input_hash"] = digest({k: v for k, v in request.items()
            if k not in {"runtime", "input_hash", "output_schema"}})
        request["output_schema"] = output_schema("synthesis", request["input_hash"],
            request["packet"]["symbols"], sorted(request["packet"]["evidence"]), request["prior_results"])
    ...
    core = {"source_run": str(source.resolve()), "source_report_sha256": file_sha256(report_path),
        ...
        "runner": runner, "timeout_seconds": 180 if diagnostic else 600, "attempts": 1,
        "purpose": "ARCHIVED_SMALL_CONTRACT_DIAGNOSTIC" if diagnostic else "FULL_SYNTHESIS_RECOVERY",
        "concise_response_requested": concise,
        "provider_output_token_limit_enforced": False,
        "budget_note": "Pi 0.85.1 Codex omits max_output_tokens on the wire; one model call, no provider retries, and parent wall deadline are enforced. Timeout usage remains unknown.",
        ...
        "runtime_dependencies": {str(p): file_sha256(p) for p in (
            ...
            Path("node_modules/@earendil-works/pi-coding-agent/dist/core/sdk.js"),
            Path("node_modules/@earendil-works/pi-coding-agent/dist/core/settings-manager.js"),
            Path("node_modules/@earendil-works/pi-coding-agent/node_modules/@earendil-works/pi-ai/dist/api/openai-codex-responses.js"))},
    ...
    for name in [*core["runtime_dependencies"], str(Path(__file__).resolve())]:
        path = Path(name)
        relative = path.resolve().relative_to(Path.cwd().resolve())
        archived = run / "registered-code" / relative
...
def main():
    ...
    if args.command in {"prepare", "prepare-diagnostic"}:
        ...
        path = prepare(args.source, args.output, args.model, diagnostic=args.command == "prepare-diagnostic",
                       concise=args.concise, symbol=args.symbol)
    else:
        if not args.run:
