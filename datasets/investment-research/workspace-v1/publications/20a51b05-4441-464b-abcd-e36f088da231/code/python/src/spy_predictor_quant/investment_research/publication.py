...
def publish(store, ledger_root, *, parent_publication_id=None):
    ...
    with store.lock(), ledger.lock():
        ...
        try:
            ...
            if affected:
                product['findings']['summary'] = ('Final refresh withdrew the prior assessment for ' + ', '.join(sorted(affected)) +
                    '. Read the remaining instrument assessments below. A new research review is required for withdrawn guidance; the original draft is retained for audit.')
            for row in product['findings']['instruments']:
                if row['symbol'] in affected:
                    row.update(assessment='insufficient_evidence', thesis='Affected guidance withdrawn after final refresh; a new research review is required.',
                               valuation='Prior valuation conclusions require review against refreshed evidence.',
                               counter_thesis='Prior counter-case requires review against refreshed evidence.',
                               review_conditions=[], claim_ids=[])
