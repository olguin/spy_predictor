...
def project(store, *, after=0, now=None):
    ...
    for identity in state['evidence_ids']:
        ...
        evidence_dates[identity] = {'observed_date': (evidence.get('data') or {}).get('price_date'),
            'published_at': evidence.get('published_at'), 'retrieved_at': evidence.get('retrieved_at')}
    ...
    for task in state['tasks']:
        ...
        tasks.append({**task, 'attempts': attempts, 'horizon_sessions': state['mandate']['horizon_sessions'],
            'evidence_dates': {i: evidence_dates[i] for i in task.get('visible_evidence_ids', []) if i in evidence_dates},
    ...
    return {'schema_version': 'investment-research-monitor-v1', 'telemetry_version': state.get('telemetry_version'),
        'run_id': state['run_id'], 'status': state['status'], 'phase': state['phase'], 'mode': state['mandate']['mode'],
        'research_question': state['mandate']['objective'],
        'symbols': [row['symbol'] for row in state['mandate']['watchlist']],
        'horizon_sessions': state['mandate']['horizon_sessions'],
